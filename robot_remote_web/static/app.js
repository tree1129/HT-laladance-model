...
async function loadConfig() {
  ...
  const policy = state.actions.policy_change_config || [];
  const waypoint = state.actions.multi_waypoint_config || [];
  const series = state.actions.series_waypoint_config || [];
...
async function sendCmdVel(vector) {
  const isStop = vector.vx === 0 && vector.vy === 0 && vector.wz === 0;
  const result = isStop
    ? await api("/api/stop")
    : await api("/api/move", { ...vector, duration: 0.24, interval: 0.06, repeat: 4 });
  return { transport: "joy_input", vector, result };
}
...
function refreshMoveLoop() {
  ...
  const tick = () => {
    ...
    sendVector(vector, `键盘移动 ${activeKeys.map((code) => keyMoveMap[code]).join("+")}`, { force: true });
  };
  tick();
